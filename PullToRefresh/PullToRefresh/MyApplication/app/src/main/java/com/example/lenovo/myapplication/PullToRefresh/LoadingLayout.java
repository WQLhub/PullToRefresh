 ...
 * Todd Davies' Progress Wheel http://github.com/Todd-Davies/ProgressWheel
 *
 ...
 */
public class LoadingLayout extends View {
  ...
  private int barWidth = 4;//外圈滚动圆环的宽度
  private int rimWidth = 4;//内圈不动圆环的宽度
  ...
  private int barColor = 0xAA000000;//外圈滚动圆环的颜色
  private int rimColor = 0x00FFFFFF;//内圈不动圆环的颜色

  //Paints
  private Paint barPaint = new Paint();//外圈画笔
  private Paint rimPaint = new Paint();//内圈画笔
  ...
  private long lastTimeAnimated = 0;//根据当前时间和lastTimeAnimated的时间差和spinSpeed这两个参数来更新滚动条的长度

  private boolean linearProgress;//控制外圈转动的类型：是否是线性变化的
  ...
  private void parseAttributes(TypedArray a) {
    ...
    circleRadius =
            (int) a.getDimension(R.styleable.ProgressWheel_header_loading_circleRadius, circleRadius);

    fillRadius = a.getBoolean(R.styleable.ProgressWheel_header_loading_fillRadius, false);

    barWidth = (int) a.getDimension(R.styleable.ProgressWheel_header_loading_barWidth, barWidth);

    rimWidth = (int) a.getDimension(R.styleable.ProgressWheel_header_loading_rimWidth, rimWidth);

    float baseSpinSpeed =
            a.getFloat(R.styleable.ProgressWheel_header_loading_spinSpeed, spinSpeed / 360.0f);
    spinSpeed = baseSpinSpeed * 360;

    barSpinCycleTime =
            a.getInt(R.styleable.ProgressWheel_header_loading_barSpinCycleTime, (int) barSpinCycleTime);

    barColor = a.getColor(R.styleable.ProgressWheel_header_loading_barColor, barColor);

    rimColor = a.getColor(R.styleable.ProgressWheel_header_loading_rimColor, rimColor);

    linearProgress = a.getBoolean(R.styleable.ProgressWheel_header_loading_linearProgress, false);

    if (a.getBoolean(R.styleable.ProgressWheel_header_loading_progressIndeterminate, false)) {
      spin();
    }
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (isSpinning) {
      ...
      if (mProgress!=mTargetProgress){
        ...
        mustInvalidate = true;
        if (isDrictToProgress){
          mProgress = mTargetProgress;//直接画到指定的弧度
        }else {
          float deltaTime = (float) (SystemClock.uptimeMillis() - lastTimeAnimated) / 1000;
          float deltaNormalized = deltaTime * spinSpeed;
          Log.d("qjb", "deltaTime:" + deltaTime);
          Log.d("qjb", "deltaNormalized:" + deltaNormalized);
          Log.d("qjb", "mTargetProgress:" + mTargetProgress);
          Log.d("qjb", "mProgress:" + mProgress);
          mProgress = Math.min(mProgress + deltaNormalized, mTargetProgress);//有一个画弧度的过程
          lastTimeAnimated = SystemClock.uptimeMillis();
        }
      }
  ...
  boolean isDrictToProgress;
  public void setProgress(float progress,boolean isDrictToProgress){
    this.isDrictToProgress = isDrictToProgress;
    if (isSpinning) {
      mProgress = 0.0f;
      isSpinning = false;
